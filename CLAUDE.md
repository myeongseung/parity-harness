...
**이 저장소는 완결됐다(2026-08-21).** 1단계(89화면 1:1 정합)와 2단계(결함 수정
D-100~D-115)가 끝났고, 디자인 변경·기능 업그레이드는 새 저장소에서 이어진다
(`migration/design/00-decisions.md` 의 종결 절 참조). 여기서 할 일은 유지·참조뿐이다 —
새 기능을 이 저장소에 더하지 않는다.
...
| **CI 커버리지** | 앱 테스트 348건 중 CI 에서 도는 것은 75건. DB 를 상대로 하는 273건은 로컬에서만 돈다 |
