...
from .model import DYNAMIC, ManifestError, Signature, load_manifest

#: 이 비율 이상 닮은 golden 항목이 있으면 발명이 아니라 라벨 드리프트로 본다.
...
#: 신규에만 있는 요소가 왜 그런지. 셋은 조치가 완전히 다르므로 섞으면 안 된다.
INVENTION = "invention"
...
def _classify(signature: Signature, golden: list[Signature]) -> tuple[str, str | None]:
    """신규에만 있는 요소를 세 갈래로 나눈다.

    - **detail_change** — 라벨이 정확히 같고 보조 키만 다르다. 링크 대상이 바뀐
      경우가 대부분이며, 라우트를 새로 설계하는 이관에서는 의도된 결과다.
    - **label_drift** — 닮은 라벨이 있다. 표기를 임의로 바꾼 것이다.
    - **invention** — 대응물이 없다. 없던 것을 만들어냈다.
    ...
    return (LABEL_DRIFT, matches[0]) if matches else (INVENTION, None)
...
def detect(
    ...
    for signature in new:
        ...
        reason, hint = _classify(signature, golden)
        if reason == INVENTION and signature.kind in _HIGH_IMPACT_KINDS:
            severity = "HIGH"
        else:
            severity = "MEDIUM"
